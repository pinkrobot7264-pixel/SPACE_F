...
| R-S16-1 | §16.1 | 200 mount/unmount cycles | `scripts/mount-stress.ps1` | 200 cycles, 100 graceful Ctrl-C + 100 forced kill | `mount-stress.txt`, every mount released, os-safety 0. Two earlier results were withdrawn: one from a harness that force-killed in both branches while reporting a split (`mount-stress-INVALID-fake-graceful-harness.txt`), one from a console-less launch whose Ctrl-C never reached a client (`mount-stress-run2-INVALID-no-console.txt`) | PASS |
