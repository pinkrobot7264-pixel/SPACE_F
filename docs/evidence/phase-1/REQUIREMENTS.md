...
| R-S11-2 | §11.2 | Suite runs with no WinFsp; separate CI job | `conformance/` | `.github/workflows/ci.yml conformance-no-winfsp` | **run `34513250205` at final HEAD `a68d3d8`**: "Assert WinFsp is NOT installed", "Build the core without WinFsp", "Conformance suite, no mount" and "Conformance suite in release (ADR-0013 panic model)" all success -- verified step-by-step via the API (`ci-verification.md`) | PASS |
...
| R-S13-3 | §13.2/§13.3 | Bounded callback under Hang, 7 assertions; repeated for 6 ops; bound scales with config | `apply_fault` Hang | `fault_tests.rs` + `scripts/fault-injection-test.ps1` | **7 of 8 rows fully PASS; 31 of 32 assertions PASS** at `353b23f` (`fault-injection-run-2026-09-10-1946.txt`, 20m14s, os-safety 0). read/write/**open**/readdir/rename + scaling (1011ms vs 1500ms bound) + §13.5 panic all PASS. `getinfo` passes a1 (24 callbacks, slowest 30011ms), a4 and a5; **a2 FAILS** -- WinFsp's `Open` returns `FSP_FSCTL_FILE_INFO` (callbacks.cpp:108) so Windows serves length from cache and never blocks on `get_file_info`. Classified EXPECTED WinFsp BEHAVIOUR in `FINDING-13-3-getinfo-assertion2.md`. Explorer-responsive remains NEEDS HUMAN | **PASS WITH DOCUMENTED ISSUE** |
...
| R-S16-5 | §16.5 | 4-hour soak with thresholds | `scripts/soak.ps1` | thresholds fixed in advance, before any numbers existed | **PASS** at `a406772`, full 240.1 min, 48 samples, zero gaps (`soak-samples.csv`, 2026-09-10 14:54-18:57). RSS 11,235,328 -> 10,698,752 (**ended below baseline**); handles 157->157; threads 7->5; 0 workload errors. Verdict: under the 1 MB/hour slope threshold, handle count stable, thread count stable. os-safety all five checks PASS | PASS (plateau criteria; see `GAP-16-5-diagnostics-endpoint.md` -- SPACE live node/handle/cursor counts were never sampled, no diagnostics endpoint exists) |
